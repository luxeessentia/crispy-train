...
export default function Home() {
  ...
  useEffect(()=> {
    // fetch products from Firestore if configured, otherwise from static JSON
    fetch('/api/products').then(r=>r.json()).then(data => setProducts(data));
    fetch('/data/reviews.json').then(r=>r.json()).then(d=> setReviews(d.filter(r=>r.approved)));
  }, []);
    ...
    <Layout>
      ...
      <section>
        ...
        <div className='carousel'>
          <div style={{width:'100%'}} className='products-grid'>{featured('Handmade Bags').map(p=> <ProductCard key={p.id} product={p} onAdd={addToCart} />)}</div>
        </div>
      </section>
      ...
      <section style={{marginTop:18}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          {reviews.slice(0,4).map((r,i)=>(<div key={i} style={{padding:12,border:'1px solid #eee',borderRadius:8}}><div style={{fontWeight:800}}>{r.name} <span style={{color:'var(--gold)'}}>{"★".repeat(r.rating)}</span></div><div style={{marginTop:8}}>{r.text}</div></div>))}
        </div>
      </section>

      <section style={{marginTop:18}}>
        <h2 className='section-title'>Newsletter</h2>
        <div style={{maxWidth:720}}><form onSubmit={(e)=>{ e.preventDefault(); alert('Thanks — subscribed!'); e.target.reset(); }} className='newsletter'><input type='email' placeholder='Your email' required style={{flex:1,padding:12,borderRadius:8,border:'1px solid #ddd'}} /><button className='btn' type='submit'>Subscribe</button></form></div>
      </section>
